<x-auth-layout>
    <x-slot name="title">Teacher Registration - {{ config('app.name', 'Laravel') }}</x-slot>

    <h3 class="text-30 lh-13">{{ __('Teacher Registration') }}</h3>
    <p class="mt-10">{{ __('Create your teacher account.') }}</p>

    <x-auth-session-status class="mb-30" :status="session('status')" />

    <form method="POST" action="{{ route('register.teacher.store') }}" class="contact-form respondForm__form row y-gap-20 pt-30">
        @csrf
