<x-dashboard-layout title="Edit Subject Category">
    <x-slot name="sidebar">
    ...
    <div class="row y-gap-20 justify-between items-end pb-30">
        <div class="col-auto">
            <h1 class="text-30 lh-12 fw-700">Edit Subject Category</h1>
            <div class="mt-10">Update the subject category details</div>
        </div>
        <div class="col-auto">
    ...
    <div class="py-30 px-30 rounded-16 bg-white -dark-bg-dark-1 shadow-4">
        <form action="{{ route('admin.subject-categories.update', $category) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row y-gap-30">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="level" id="level" class="form-select @error('level') is-invalid @enderror" required>
                            ...
                            <option value="Junior Secondary" {{ old('level', $category->level) == 'Junior Secondary' ? 'selected' : '' }}>Junior Secondary (Grades 7–9)</option>
                            <option value="Senior School" {{ old('level', $category->level) == 'Senior School' ? 'selected' : '' }}>Senior School (Grades 10–12)</option>
                        </select>
                        @error('level')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" name="position" id="position" class="form-control @error('position') is-invalid @enderror" value="{{ old('position', $category->position) }}" min="0">
                        @error('position')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
