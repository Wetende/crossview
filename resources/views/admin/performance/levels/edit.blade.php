<x-dashboard-layout>
    ...
    <div class="dashboard__content bg-light-4">
        <div class="row y-gap-20 justify-between items-end pb-30 mb-10">
            <div class="col-auto">
                <h1 class="text-30 lh-12 fw-700">Edit Performance Level</h1>
                <div class="mt-10">Update details for "{{ $level->name }}"</div>
            </div>
            <div class="col-auto">
        ...
        <div class="row y-gap-30">
            <div class="col-12">
                <div class="py-30 px-30 rounded-16 bg-white -dark-bg-dark-1 shadow-4">
                    ...
                    <form action="{{ route('admin.performance.levels.update', $level) }}" method="POST" class="row y-gap-20">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="d-flex items-center">
                                <input type="color" id="color_picker" class="form-control-color mr-10" value="{{ old('color_code', $level->color_code) }}" onchange="document.getElementById('color_code').value = this.value;">
                                <input type="text" name="color_code" id="color_code" class="form-control @error('color_code') is-invalid @enderror" value="{{ old('color_code', $level->color_code) }}" required>
                            </div>
                            @error('color_code')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-lg-6">
                            ...
                            <input type="number" name="min_score" id="min_score" step="0.01" min="0" max="100" class="form-control @error('min_score') is-invalid @enderror" value="{{ old('min_score', $level->min_score) }}" required>
                            @error('min_score')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-lg-6">
                            ...
                            <input type="number" name="max_score" id="max_score" step="0.01" min="0" max="100" class="form-control @error('max_score') is-invalid @enderror" value="{{ old('max_score', $level->max_score) }}" required>
                            @error('max_score')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="description" id="description" rows="4" class="form-control @error('description') is-invalid @enderror">{{ old('description', $level->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <input type="number" name="display_order" id="display_order" class="form-control @error('display_order') is-invalid @enderror" value="{{ old('display_order', $level->display_order) }}" min="1">
                            @error('display_order')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12 mt-20">
                            ...
                            <div class="py-15 px-20 rounded-8 border-light" id="level_preview" style="border-left: 4px solid {{ old('color_code', $level->color_code) }}; background-color: {{ old('color_code', $level->color_code) }}10;">
                                <div id="preview_name" class="text-16 fw-500" style="color: {{ old('color_code', $level->color_code) }}">{{ old('name', $level->name) }}</div>
                                <div class="d-flex items-center mt-5">
                                    <div class="text-14 mr-10">Score Range:</div>
                                    <div class="text-14 fw-500" id="preview_range">{{ old('min_score', $level->min_score) }} - {{ old('max_score', $level->max_score) }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-12 mt-20">
                            ...
                            <div class="d-flex items-center justify-end">
                                ...
                                <button type="submit" class="button -md -purple-1 text-white">Update Level</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updatePreview() {
                ...
                preview.style.backgroundColor = color + '10'; /* Appending '10' for ~6% opacity */
            ...
            colorInput.addEventListener('input', function() {
                ...
                if (/^#[0-9A-F]{6}$/i.test(this.value) || /^#[0-9A-F]{3}$/i.test(this.value)) {
                    colorPicker.value = this.value;
                } else {
                    // Revert to color picker's current value if input is invalid to avoid breaking it
