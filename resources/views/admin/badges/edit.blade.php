<x-dashboard-layout>
    ...
    <div class="dashboard__content bg-light-4 py-30 px-30">
        <div class="row y-gap-20 justify-between items-end pb-30">
            <div class="col-auto">
                <h1 class="text-30 lh-12 fw-700">Edit Badge: {{ $badge->name }}</h1>
                <div class="mt-10">Update badge details and criteria.</div>
            </div>
            <div class="col-auto">
                ...
                <div class="py-30 px-30 rounded-16 bg-white -dark-bg-dark-1 shadow-4">
                    <form action="{{ route('admin.badges.update', $badge) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row y-gap-30">
                            <div class="col-md-8">
                                <div class="row y-gap-20">
                                    <div class="col-12">
                                        ...
                                        <input type="text" name="name" id="name" value="{{ old('name', $badge->name) }}" class="form-control @error('name') is-invalid @enderror" required>
                                        @error('name')
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" name="slug" id="slug" value="{{ old('slug', $badge->slug) }}" class="form-control @error('slug') is-invalid @enderror">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea name="description" id="description" rows="4" class="form-control @error('description') is-invalid @enderror" required>{{ old('description', $badge->description) }}</textarea>
                                        @error('description')
                                            <div class="text-red-1 mt-1">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" name="points" id="points" value="{{ old('points', $badge->points) }}" min="0" class="form-control @error('points') is-invalid @enderror" required>
                                        @error('points')
                                            <div class="text-red-1 mt-1">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="criteria_type" id="criteria_type" class="form-select @error('criteria_type') is-invalid @enderror">
                                            ...
                                            <option value="course_completion" {{ old('criteria_type', $badge->criteria_type) == 'course_completion' ? 'selected' : '' }}>@lmsterm('Study Material') Completion</option>
                                            <option value="quiz_score" {{ old('criteria_type', $badge->criteria_type) == 'quiz_score' ? 'selected' : '' }}>Quiz Score</option>
                                            <option value="forum_posts" {{ old('criteria_type', $badge->criteria_type) == 'forum_posts' ? 'selected' : '' }}>Forum Posts</option>
                                            <option value="manual" {{ old('criteria_type', $badge->criteria_type) == 'manual' ? 'selected' : '' }}>Manual Award</option>
                                        </select>
                                        @error('criteria_type')
                                            <div class="text-red-1 mt-1">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="criteria_value" id="criteria_value" value="{{ old('criteria_value', $badge->criteria_value) }}" class="form-control @error('criteria_value') is-invalid @enderror">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-switch d-flex items-center mt-20">
                                            <div class="switch">
                                                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $badge->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-4">
                                <div class="d-flex flex-column items-center">
                                    ...
                                    <div class="badge-preview size-120 rounded-16 mb-15 d-flex justify-center items-center {{ $badge->icon_path ? '' : 'bg-light-4' }}">
                                        @if($badge->icon_path)
                                            <img src="{{ $badge->icon_url }}" alt="{{ $badge->name }}" class="size-100 rounded-8 object-cover">
                                        @else
                        ...
                        <div class="row y-gap-20 justify-between pt-30">
                            <div class="col-auto">
                                <button type="submit" class="button -md -purple-1 text-white">Update Badge</button>
                            </div>
                            <div class="col-auto">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (nameInput && slugInput) {
                // Only set auto-generated to true if the slug matches a slug pattern
                const slugPattern = /^[a-z0-9-]+$/;
                const originalSlug = '{{ $badge->slug }}';
                
                // Initialize the dataset attribute
                slugInput.dataset.autoGenerated = originalSlug && slugPattern.test(originalSlug) ? 'true' : 'false';
                ...
                iconInput.addEventListener('change', function(e) {
                    ...
                    if (file && file.type.match('image.*')) {
                        ...
                        reader.onload = function(e) {
                            // Replace the current icon with the uploaded image
                            previewContainer.innerHTML = '';
                            const img = document.createElement('img');
                            img.src = e.target.result;
                            img.className = 'size-100 rounded-8 object-cover';
                            previewContainer.appendChild(img);
                            previewContainer.classList.remove('bg-light-4');
                        };
