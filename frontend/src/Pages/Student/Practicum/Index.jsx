...
function SubmissionCard({ submission }) {
  const handleDownload = async () => {
    try {
      const response = await axios.get(`/api/v1/student/practicum/${submission.id}/download/`);
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {submission.programName} • Version {submission.version}
              </Typography>
            </Box>
          ...
          {submission.review && (
            <Box sx={{ mt: 2, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
              ...
              {submission.review.comments && (
                <Typography variant="body2">{submission.review.comments}</Typography>
              )}
              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
                Reviewed: {new Date(submission.review.reviewedAt).toLocaleDateString()}
              </Typography>
            </Box>
          )}
    ...
    <>
      ...
      <Stack spacing={3}>
        ...
        {submissions.length === 0 ? (
          <motion.div {...fadeIn}>
            <Alert severity="info">
              No practicum submissions found. Submit your first practicum from a practicum node in your program.
            </Alert>
          </motion.div>
        ) : (
          <>
