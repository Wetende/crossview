  ...
  const handleDownload = async () => {
    try {
      ...
      if (error.response?.status === 403) {
        alert('This certificate has been revoked and cannot be downloaded.');
      }
    }
  };
        ...
        {certificates.length === 0 ? (
          <motion.div {...fadeIn}>
            <Alert severity="info" icon={<VerifiedIcon />}>
              You haven't earned any certificates yet. Complete a program to receive your certificate!
            </Alert>
          </motion.div>
        ) : (
