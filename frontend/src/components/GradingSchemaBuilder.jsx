...
const GRADING_TYPES = [
  { value: 'weighted', label: 'Weighted Average', description: 'Multiple components with percentage weights (e.g., CAT 30%, Exam 70%)' },
  { value: 'competency', label: 'Competency-Based', description: 'Competent / Not Yet Competent assessment' },
  { value: 'rubric', label: 'Rubric (CBC Style)', description: '4-point scale with criteria' },
  { value: 'percentage', label: 'Simple Percentage', description: 'Single 0-100 score' },
  { value: 'checklist', label: 'Checklist', description: 'Pass/Fail per item' },
  { value: 'pass_fail', label: 'Pass/Fail', description: 'Simple pass or fail' },
];
...
function WeightedBuilder({ components = [], passMark, onChange }) {
  const [newComponent, setNewComponent] = useState({ key: '', label: '', weight: 0 });
  ...
  const addComponent = () => {
    if (newComponent.label.trim() && newComponent.weight > 0) {
      ...
      onChange({
        components: [...components, { ...newComponent, key, weight: parseFloat(newComponent.weight) / 100 }],
      });
        ...
        <Box sx={{ bgcolor: 'grey.50', borderRadius: 1, p: 2 }}>
          {components.map((comp, index) => (
            <Box
              ...
              sx={{
                ...
                gap: 2,
                py: 1,
                borderBottom: index < components.length - 1 ? '1px solid' : 'none',
                borderColor: 'divider',
              }}
            >
              <DragIndicatorIcon color="action" />
              <Typography sx={{ flex: 1 }}>{comp.label}</Typography>
              <Chip
          ...
          <Box sx={{ mt: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="body2">
              Total: <strong>{(totalWeight * 100).toFixed(0)}%</strong>
            </Typography>
            {!isValid && totalWeight > 0 && (
              <Chip
                label={totalWeight < 1 ? `Need ${((1 - totalWeight) * 100).toFixed(0)}% more` : 'Over 100%'}
                size="small"
                color="error"
              />
            )}
      ...
      <Grid container spacing={2} alignItems="flex-end">
        ...
        <Grid item xs={4}>
          <Button
            ...
            disabled={!newComponent.label.trim() || !newComponent.weight}
            fullWidth
          >
            Add
          </Button>
...
function CompetencyBuilder({ levels = [], components = [], onChange }) {
  ...
  const addLevel = () => {
    if (newLevel.label.trim()) {
      ...
      onChange({ levels: [...(levels.length > 0 ? levels : defaultLevels), { ...newLevel, key }] });
    ...
    <Stack spacing={3}>
      ...
      <Box>
        ...
        <Stack spacing={1} sx={{ mb: 2 }}>
          {(levels.length > 0 ? levels : defaultLevels).map((level, index) => (
            <Box
              key={index}
              sx={{
                display: 'flex',
            ...
            >
              <Typography sx={{ flex: 1 }}>{level.label}</Typography>
              <Chip
                label={level.passing ? 'Passing' : 'Not Passing'}
                size="small"
      ...
      <Box>
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mb: 2 }}>
          {components.map((comp, index) => (
            <Chip
              key={index}
              label={comp}
              onDelete={() => onChange({ components: components.filter((_, i) => i !== index) })}
            />
          ))}
        </Stack>
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          />
          <Button variant="outlined" onClick={addComponent} disabled={!newComponent.trim()}>
            Add
          </Button>
        </Box>
      </Box>
          ...
          {(levels.length > 0 ? levels : defaultLevels).map((level, index) => (
            <Grid item xs={6} sm={3} key={index}>
              <Box sx={{ textAlign: 'center' }}>
                <Chip label={level.score} color="primary" sx={{ mb: 1 }} />
                <TextField
                  value={level.label}
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mb: 2 }}>
          {criteria.map((c, index) => (
            <Chip
              key={index}
              label={c}
              onDelete={() => onChange({ criteria: criteria.filter((_, i) => i !== index) })}
            />
          ))}
        </Stack>
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          />
          <Button variant="outlined" onClick={addCriterion} disabled={!newCriterion.trim()}>
            Add
          </Button>
        </Box>
      </Box>
...
function ChecklistBuilder({ items = [], onChange }) {
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="subtitle2" color="text.secondary">
        Add items that students must pass (all items required for overall pass)
      </Typography>
      ...
      <Box sx={{ display: 'flex', gap: 1 }}>
        ...
        />
        <Button variant="outlined" onClick={addItem} disabled={!newItem.trim()}>
          Add
        </Button>
      </Box>
    </Stack>
  ...
  return (
    <Box>
      ...
      <TextField
        ...
        type="number"
        value={passMark || 50}
        onChange={(e) => onChange({ pass_mark: parseInt(e.target.value) || 0 })}
        inputProps={{ min: 0, max: 100 }}
        size="small"
      />
    </Box>
...
function PassFailBuilder() {
  return (
    <Alert severity="info">
      Students will receive a simple Pass or Fail result. No additional configuration needed.
    </Alert>
  );
}
