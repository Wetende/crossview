...
const getIconForType = (type, lessonType) => {
    ...
    if (t === 'quiz' || lt === 'quiz') return <QuizIcon sx={{ color: 'warning.main' }} />;
    if (t === 'assignment' || lt === 'assignment') return <AssignmentIcon sx={{ color: 'error.main' }} />;
    if (lt === 'video') return <VideoIcon sx={{ color: 'info.main' }} />;
    if (lt === 'live_class') return <ZoomIcon sx={{ color: 'secondary.main' }} />;
...
export default function SearchMaterialsModal({ 
    ...
    const filteredMaterials = useMemo(() => {
        return materials.filter(material => {
            ...
            const materialType = (material.type || material.node_type || '').toLowerCase();
            const lessonType = material.properties?.lesson_type || '';
            
            let matchesType = typeFilter === 'all';
            if (typeFilter === 'lesson') {
                matchesType = materialType === 'lesson' && !['quiz', 'assignment'].includes(lessonType);
            } else if (typeFilter === 'quiz') {
                matchesType = materialType === 'quiz' || lessonType === 'quiz';
            } else if (typeFilter === 'assignment') {
                matchesType = materialType === 'assignment' || lessonType === 'assignment';
            }
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            if (data.success) {
                handleClose();
                // Refresh the page to show imported materials
                if (onImportComplete) {
                    onImportComplete(data.imported_count);
                } else {
                ...
                {loading && materials.length === 0 ? (
                    ...
                    <List disablePadding sx={{ maxHeight: 300, overflowY: 'auto' }}>
                        {filteredMaterials.map((material) => {
                            ...
                            const materialType = material.type || material.node_type || 'Lesson';
                            const lessonType = material.properties?.lesson_type || '';
                                ...
                                <ListItem key={material.id} disablePadding>
                                    ...
                                    >
                                        <ListItemIcon sx={{ minWidth: 40 }}>
                                            {getIconForType(materialType, lessonType)}
                                        </ListItemIcon>
                                        <ListItemText 
                                            primary={material.title}
                                            secondary={
                                                <Chip 
                                                    label={material.program_name} 
                                                    size="small" 
                                                    variant="outlined"
