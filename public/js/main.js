...
const ShowMore = (function() {
  function init() {
    ...
    targets.forEach((el) => {
      ...
      // Skip if either button or content is missing
      if (!button || !content) {
        console.warn('Show more component missing required elements', el);
        return;
      }
...
// Curriculum reordering functionality (for course sections and content)
document.addEventListener('DOMContentLoaded', function() {
    const sectionsContainer = document.getElementById('courseSectionsAccordion');
    
    if (sectionsContainer) {
        // Enable drag-and-drop reordering for sections
        new Sortable(sectionsContainer, {
            animation: 150,
            handle: '.drag-handle',
            ghostClass: 'bg-light-4',
            onEnd: function(evt) {
                const sectionIds = Array.from(sectionsContainer.querySelectorAll('.section-item'))
                    .map(item => item.dataset.sectionId);
                
                // Send the reordered sections to the server
                const reorderUrl = sectionsContainer.dataset.reorderUrl;
                fetch(reorderUrl, {
                    method: 'POST',
                    headers: {
        ...
        // Enable drag-and-drop reordering for content within each section
        document.querySelectorAll('.section-content-list').forEach(contentList => {
            new Sortable(contentList, {
                ...
                onEnd: function(evt) {
                    ...
                    // Send the reordered content to the server
                    const reorderUrl = contentList.dataset.reorderUrl;
                    fetch(reorderUrl, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    .then(response => response.json())
                    .then(data => {
                        console.log('Section content reordered successfully:', data);
